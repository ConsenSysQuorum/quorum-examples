const Web3 = require("web3");

const wsProvider = new Web3.providers.WebsocketProvider("ws://localhost:8547", {
    headers: {
        Origin: "http://localhost"
    }
});
const web3 = new Web3(wsProvider);


const simpleStorageDeploy = "";

const abi = [{
    constant: true,
    inputs: [],
    name: "storedData",
    outputs: [{
        name: "",
        type: "uint256"
    }],
    payable: false,
    type: "function"
}, {
    constant: false,
    inputs: [{
        name: "x",
        type: "uint256"
    }],
    name: "set",
    outputs: [],
    payable: false,
    type: "function"
}, {
    constant: true,
    inputs: [],
    name: "get",
    outputs: [{
        name: "retVal",
        type: "uint256"
    }],
    payable: false,
    type: "function"
}, {
    inputs: [{
        name: "initVal",
        type: "uint256"
    }],
    payable: false,
    type: "constructor"
}];

const simpleContract = new web3.eth.Contract(abi);

const initializedDeploy = simpleContract
    .deploy({
        data: simpleStorageDeploy,
        arguments: [42]
    })
    .encodeABI();

const accAddress = "0x0638e1574728b6d862dd5d3a3e0942c3be47d996";// node 5
//const accAddress = "0xed9d02e382b34818e88b88a309c7fe71e65f419d"; // node 1


const txnParams = {
    gasPrice: 0,
    gasLimit: 4300000,
    to: null,
    value: 0,
    data: initializedDeploy,
    from: accAddress
};

const TOTAL_TXN = 50000;
const MAX_OUTSTANDING = 5000;
const MIN_OUTSTANDING = 2500;

var MIN_OUTSTANDING_GAS = null;

var txCount = 0;
var successfullySentCounter = 0;
var TX_GAS = null;
var outstandingGas = 0;

var blockSubscription = null;

var startTime = null;


function initAfterGasEstimate(txGasValue) {
    TX_GAS = txGasValue;
    console.log("TXGasEstimate: " + txGasValue);
    MIN_OUTSTANDING_GAS = MIN_OUTSTANDING * txGasValue;

    console.log("Starting test. TotalTxToSend: " + TOTAL_TXN);
    startTime = Date.now();

    blockSubscription = web3.eth.subscribe('newBlockHeaders')
        .on("data", onNewBlock)
        .on("error", console.error);
}

function onNewBlock(blockHeader) {
    blockTxCount = blockHeader.gasUsed / TX_GAS;
    console.log("Block No: " + blockHeader.number + " Estimated tx in block: " + blockTxCount);
    outstandingGas = outstandingGas - blockHeader.gasUsed;
    if (txCount < TOTAL_TXN) {
        if (outstandingGas < MIN_OUTSTANDING_GAS) {
            send();
        }
    } else if (outstandingGas === 0){
        blockSubscription.unsubscribe(function (error, success) {
            if (success){
                console.log("Successfully unsubscribed!");
                endTime = Date.now();
                tps = TOTAL_TXN * 1000 / (endTime-startTime);
                console.log("Average TPS: " + tps);
                console.log("End txCount: " + txCount);
                process.exit();
            }
        })
    }
}

function send() {
    toSend = MAX_OUTSTANDING;
    if (txCount + toSend > TOTAL_TXN) {
        toSend = TOTAL_TXN - txCount;
    }
    for (i = 0; i < toSend; i++) {
        web3.eth.sendTransaction(txnParams, function (error, result) {
            if (error) {
                console.log("An error occurred while trying to send a transaction.");
                txCount = txCount - 1;
                outstandingGas = outstandingGas - TX_GAS;
            }
            successfullySentCounter = successfullySentCounter + 1;
            if (successfullySentCounter % 500 === 0) {
                console.log("successfullySentCounter: " + successfullySentCounter);
            }

        });
        txCount = txCount + 1;
        outstandingGas = outstandingGas + TX_GAS;
    }
}

web3.eth.estimateGas(txnParams).then(initAfterGasEstimate);
