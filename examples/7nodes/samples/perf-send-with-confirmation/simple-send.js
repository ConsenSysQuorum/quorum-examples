const Web3 = require("web3");

const wsProvider = new Web3.providers.WebsocketProvider("ws://localhost:8546", {
    headers: {
        Origin: "http://localhost"
    }
});
const web3 = new Web3(wsProvider);




  const simpleStorageDeploy = "";

  const abi = [{
    constant: true,
    inputs: [],
    name: "storedData",
    outputs: [{
      name: "",
      type: "uint256"
    }],
    payable: false,
    type: "function"
  }, {
    constant: false,
    inputs: [{
      name: "x",
      type: "uint256"
    }],
    name: "set",
    outputs: [],
    payable: false,
    type: "function"
  }, {
    constant: true,
    inputs: [],
    name: "get",
    outputs: [{
      name: "retVal",
      type: "uint256"
    }],
    payable: false,
    type: "function"
  }, {
    inputs: [{
      name: "initVal",
      type: "uint256"
    }],
    payable: false,
    type: "constructor"
  }];

  const simpleContract = new web3.eth.Contract(abi);

  const initializedDeploy = simpleContract
    .deploy({
      data: simpleStorageDeploy,
      arguments: [42]
    })
    .encodeABI();

  const accAddress = "0xed9d02e382b34818e88b88a309c7fe71e65f419d";


  const txnParams = {
    gasPrice: 0,
    gasLimit: 4300000,
    to: null,
    value: 0,
    data: initializedDeploy,
    from: accAddress
  };

  const TOTAL_TXN = 50000;
  const MAX_PARALLEL = 1500;

  var txCount = 0;



function send() {
    console.log("Txn Confirmation blocks" + web3.eth.transactionConfirmationBlocks);
    console.log("Txn block timeout" + web3.eth.transactionBlockTimeout);
    console.log("Txn poll timeout" + web3.eth.transactionPollingTimeout);
    web3.eth.estimateGas(txnParams).then(console.log);

    // web3.eth.sendTransaction(txnParams, function(error, hash){
    //     console.log(error);
    //     console.log(hash);
    //     process.exit();
    // });

}


send();
