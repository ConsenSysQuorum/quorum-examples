...
contract VoterManager {
    enum PendingOpType {None, OrgAdd, OrgSuspension, OrgRevokeSuspension, AddOrgAdmin}
    ...
    function addVotingItem(string calldata _authOrg, string calldata _orgId, string calldata _enodeId, address _account, PendingOpType _pendingOp) external {
        // check if anything is pending approval for the org. If yes another item cannot be added
        require(checkPendingOp(_authOrg) != true, "Items pending approval. New item cannot be added");
    ...
    function processVote(string calldata _authOrg, address _vAccount) external voterExists(_authOrg, _vAccount) returns (bool) {

        // check something is pending approval
        require(checkPendingOp(_authOrg) == true, "nothing to approve");
        uint id = getVoterOrgIndex(_authOrg);
    ...
    function checkPendingOp(string memory _orgId) internal view returns (bool){
        uint orgIndex = getVoterOrgIndex(_orgId);
        return (orgVoterList[orgIndex].pendingOp.opType != PendingOpType.None);
    }
    ...
    function getPendingOpDetails(string calldata _orgId) external view returns (string memory, string memory, address, PendingOpType){
        uint orgIndex = getVoterOrgIndex(_orgId);
